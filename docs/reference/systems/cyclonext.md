...
Write commands are sent as JSON frames via the shared WebSocket transport (`AqualinkClient.send_ws_frame`), with frames built by `iaqualink.shared.robots`. The action is `setCleanerState` with namespace `cyclonext`. Equipment payload is wrapped in the literal string key `"robot.1"` (with a dot — cyclonext-specific).
