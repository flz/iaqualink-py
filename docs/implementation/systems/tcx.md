...
`lvh1` ("light/valve/heater 1", the pib0-namespace heater-tile object — see protocol reference) is modeled as **two sibling devices**, not folded into `TcxClimate`: `TcxHeaterStatusSensor` (`AqualinkSensor`, canonical key `lvh1`) and `TcxHeaterEnableSwitch` (`AqualinkSwitch`, synthetic key `lvh1_enable`), both built from the same `reported["lvh1"]` dict in `_update_devices()` — the same synthetic-sibling technique already used for `ecm0`/`ecm0_minSpd` etc.
...
`TcxAuxLight.is_on`/`turn_on`/`turn_off` reuse the existing, already-confirmed `set_aux(name, state)` (`aux0.st`) — same mechanism as `TcxAuxSwitch`. Color control is now named-effect based, mirroring iaqua's `IaquaColorLight*` pattern: `_TCX_LIGHT_COLORS` (`src/iaqualink/systems/tcx/device.py`) maps each color-capable `LightType` (`JL`/`PSS`/`IB`/`HU`) to its own fixed, ordered color-name tuple, confirmed via external protocol research against TCX's own equipment types (not borrowed from iaqua's unrelated per-brand lists, which are keyed on a different platform's `subtype` codes and don't line up 1:1 — e.g. TCX's `HU` "Hayward Universal ColorLogic" has no iaqua equivalent at all). The wire index (`cmdClr`/`currClr`/`rstClr`) is 1-based against list position. `effect`/`effect_list` are backed by this mapping; `current_color_index`/`set_color_index()` remain available for callers that want the raw index directly. Unlike iaqua's `IaquaColorLight.effect` (which can only report on/off — the iaqua API has no readback path), TCX's `currClr` field means `effect` reports the actual active color name.

`set_color_index()` calls `set_aux_light` (`"setAuxLight"`, `NAMESPACE_PIB`, `{name: {"cmdClr": index}}`). `reset_color()` calls `reset_aux_light` (`"setAuxResetColor"`, `NAMESPACE_PIB`, `{name: {"rstClr": index}}`) with the *current* color index — confirmed wire behavior re-selects the already-active color rather than clearing one, so `reset_color()` is a no-op when `current_color_index` is unknown (nothing to re-select). Both actions were originally modeled under `NAMESPACE_TCX` with an empty `reset_aux_light` payload; both were corrected once external protocol research confirmed the `pib` namespace and the `rstClr` payload shape.
...
### Speed fields intentionally not modeled

`filt0.manSpd`/`ecm0.manSpd`/`ecm0.reqSpd`/`filt0.sp`/`ecm0.qcSpd`/`ecm0.qcDur` are **not modeled as devices at all** — no `TcxSpeedSensor`-style class remains in this codebase (it was removed as dead code once its last user, `qcSpd`, was dropped). Per user-supplied research into the reference app: `manSpd` (both `filt0` and `ecm0`), `reqSpd`, and `filt0.sp` are neither displayed nor written by the app (see the "Reference app display/write behavior" table in `docs/reference/systems/tcx.md`); `qcSpd`/`qcDur` have documented actions (`setQuickCleanSpeed`/`setQuickCleanDuration`) but quick-clean is confirmed unused by TCX firmware. `cmdSpd` is deliberately **not** duplicated as a standalone sensor either — already backs `percentage`/`preset_mode` (see "VSP speed commands" above).
...
`ecm0.minSpd`/`maxSpd`/`prmSpd`/`frzSpd`/`prmDur` each have a confirmed `vsp`-namespace write action (`setMinMasterSpeed`/`setMaxMasterSpeed`/`setPrimingSpeed`/`setFreezeProtectSpeed`/`setPrimingSpeedDuration`) and are modeled as `AqualinkNumber` devices, synthesized in `_update_devices()` under synthetic keys (`ecm0_minSpd` etc., `_ECM0_WRITABLE_SPEED_FIELDS` in `system.py`) — each candidate carries the *full* `ecm0` dict (not just the one field), so `TcxVspMinSpeed`/`TcxVspMaxSpeed` can read each other's current value for the `minSpd <= maxSpd` invariant, and `TcxVspPrimingSpeed`/`TcxVspFreezeProtectSpeed` can read the dynamic `[minSpd, maxSpd]` bounds — all per a user-supplied device spec (ranges/steps not in the reference doc's original field table, supplied via external protocol research like `AuxType` above).
...
| 10 | Temperature wire fields (`waterTempSet`, `water.value`, `solar.value`, `freezeSP`, `lowAirSP`) assumed to be whole degrees in the active unit | Live wire capture shows they're tenths of a degree instead — confirmed by `freezeSP=33`/`lowAirSP=128` only making sense as `3.3`/`12.8`, and `waterTempSet`/`water.value` agreeing at raw `283` only making sense as `28.3°C`. Fixed for water/solar sensors and climate current/target temperature (read and write). `freezeSP` is now surfaced as `TcxFreezeSetPoint` (see deltas #18/#19) and receives the same tenths-of-a-degree scaling on read/write; `lowAirSP` remains unmodeled and needed no change |
...
| 15 | `set_lvh_app_type`/`set_aux_setup`/`set_aux_freeze_protect` payload shapes inferred by reusing the same "wire field name -> desired-state delta" convention as delta #7 | Action names (`setLvhAppType`/`setAuxSetup`/`setIsAux0FreezeProtect`) and their namespace (`tcx`/`NAMESPACE_TCX`) are confirmed from the Command Reference table; no field-level payload example exists for any of the three. `set_aux_freeze_protect(enabled)` takes no aux-name parameter — target is hardcoded to `aux0`, matching the action name itself (see "Aux freeze protect" above). ~~`set_aux_light`/`reset_aux_light` payload shapes and namespace~~ — **resolved**, see delta #16 |
| 16 | ~~`TcxAuxLight` exposes raw `currClr`/`cmdClr` color-index control only, no named `effect`/`effect_list`; `set_aux_light`/`reset_aux_light` assumed `NAMESPACE_TCX` with an empty `reset_aux_light` payload~~ — **resolved** | External protocol research confirmed: (1) both actions use `NAMESPACE_PIB` (`"pib"`), not `NAMESPACE_TCX`; (2) `reset_aux_light`'s delta carries `{"rstClr": index}` (the current color's index), not an empty object; (3) a fixed, ordered, TCX-specific color-name list exists per color-capable `LightType` (`JL`/`PSS`/`IB`/`HU`), 1-based against `cmdClr`/`currClr`/`rstClr` — now backing `TcxAuxLight.effect`/`effect_list` via `_TCX_LIGHT_COLORS` (see "Aux light control" above) |
...
| 25 | `ecm0.qcSpd`/`qcDur` (`setQuickCleanSpeed`/`setQuickCleanDuration`) not wired up despite documented actions | Confirmed unused by TCX firmware per the same user-supplied spec as delta #22 — neither is surfaced as a device |
| 26 | `filt0.manSpd`/`ecm0.manSpd`/`ecm0.reqSpd`/`filt0.sp` intentionally not modeled as devices at all — `TcxSpeedSensor` removed entirely (dead code once `qcSpd`, its last user, was also dropped) | Confirmed via user-supplied research into the reference app that none of these fields are displayed or written by the app — see "Reference app display/write behavior" in the protocol reference |
