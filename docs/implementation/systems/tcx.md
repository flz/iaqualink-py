...
| `ecm0.manSpd`/`qcSpd` (synthetic keys `ecm0_manSpd`, `ecm0_qcSpd`) | `TcxSpeedSensor` | `AqualinkSensor` | Raw RPM, read-only — no confirmed write action for `manSpd`; `qcSpd` has one (`setQuickCleanSpeed`) but is confirmed unused by TCX firmware. `cmdSpd` intentionally not duplicated as a sensor — already backs `TcxVariableSpeedPump.percentage`/`preset_mode` |
...
Live wire capture shows `filt0` and `ecm0` both carry raw speed (RPM) fields beyond what `TcxVariableSpeedPump` already surfaces via `percentage`/`preset_mode`: `filt0.manSpd`, `ecm0.manSpd`, `ecm0.qcSpd` (quick-clean). These have no confirmed write action (`qcSpd`'s `setQuickCleanSpeed` exists but is confirmed unused by TCX firmware — see "VSP speed/duration set points" below) and are exposed as standalone read-only `TcxSpeedSensor` devices, synthesized in `_update_devices()` under synthetic keys (`filt0_manSpd`, `ecm0_manSpd`, `ecm0_qcSpd`) — not real wire device names, so `TcxDevice.from_data()` dispatches them by exact match against `_SPEED_SENSOR_NAMES` rather than the `filt0`/`ecm0` name branches. `ecm0.frzSpd`/`prmSpd` used to be modeled this way too — see below, now writable.

`filt0.manSpd` and `ecm0.manSpd` are **not duplicates** — live-confirmed distinct values (`2000` vs `1900` on the same real system) — so both get separate sensors rather than being merged into one. `cmdSpd`/`reqSpd` are deliberately **not** duplicated as standalone sensors: `cmdSpd` already backs `percentage`/`preset_mode`; `reqSpd` is a near-duplicate of `cmdSpd`.
