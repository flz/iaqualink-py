...
All write commands go through the shared WebSocket `_robot_ws`. Commands wrap their equipment state in `{"robot": {...}}` (literal `"robot"` key — no dot, unlike cyclonext's `"robot.1"`).
