"""i2d_robot wire-format parser for the 36-char hex status string."""

from __future__ import annotations

__all__ = ["EXPECTED_HEX_LEN", "I2dStatus", "parse_status_hex"]
...
class I2dStatus:
    ...
    def state_label(self) -> str:
        return I2D_STATE_LABELS.get(
            self.state_code, f"unknown_{self.state_code:02X}"
        )

    @property
    def error_label(self) -> str:
        return I2D_ERROR_LABELS.get(
            self.error_code, f"unknown_{self.error_code:02X}"
        )

    @property
    def mode_label(self) -> str:
        return I2D_MODE_LABELS.get(
            self.mode_code, f"unknown_{self.mode_code:02X}"
        )


def parse_status_hex(hex_str: str) -> I2dStatus:
    """Parse the 36-char hex status response into a typed I2dStatus.
