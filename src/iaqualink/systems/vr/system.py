...
# many internal/config scalars that would flood HA with noise entities, so
# only the operationally meaningful scalars are allowlisted; errorState is
# exposed separately as the snake_case `error_state` sensor.
...
class VrSystem(AqualinkSystem):
    ...
    def _parse_shadow_response(self, response: httpx.Response) -> None:
        ...
        # also carries config/internal scalars we deliberately don't expose;
        # errorState is surfaced below as the snake_case `error_state` sensor.
