"""VR write-side WebSocket commands.

Namespace is parametric so VortraxSystem (a VrSystem subclass) can reuse
these helpers with `namespace="vortrax"` instead of `"vr"`.
"""
...
async def send_set_state(
    ...
    *,
    ...
    frame = build_robot_state_frame(
        serial, {"state": state}, client_token(client), namespace=namespace
    )
...
async def send_set_cycle(
    ...
    *,
    ...
    frame = build_robot_state_frame(
        serial, {"prCyc": cycle}, client_token(client), namespace=namespace
    )
    await send_robot_frame(client, frame)
...
async def send_set_stepper(
    ...
    *,
    ...
    frame = build_robot_state_frame(
        serial, {"stepper": minutes}, client_token(client), namespace=namespace
    )
    await send_robot_frame(client, frame)
...
async def send_remote_steering(
    ...
    *,
    ...
    # Caller must put the robot in pause state (state==2) first;
