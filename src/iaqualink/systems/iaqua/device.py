...
class IAqualinkNumber(AqualinkDevice):
...
class IAquaAquapureProductionSwitch(AqualinkToggle):
...
class IAquaAquapureBoostSwitch(AqualinkToggle):
    ...
    async def refresh(self) -> None:
        shadow = await self._client.get_device_shadow(self.serial)
        eq = _get_nested(shadow, _SWC_REPORTED_PATH, {}) or {}
        self._data.update({"boost": eq.get("boost")})


# ---------------- Number: production level 0–100% ----------------
class IAquaAquapureProductionLevelNumber(IAqualinkNumber):
    """Production percentage (0..100%)."""
    device_type = "aquapure_production_number"
    min_value = 0.0
    max_value = 100.0
    step = 1.0
    unit_of_measurement = "%"
    ...
    async def set_value(self, value: float) -> None:
        ...
        if v > self.max_value:
            v = self.max_value
        await self._client.patch_device_shadow(self.serial, _desired(production=int(v)))
        await self.refresh()

    async def refresh(self) -> None:
        shadow = await self._client.get_device_shadow(self.serial)
        eq = _get_nested(shadow, _SWC_REPORTED_PATH, {}) or {}
        self._data.update({"production": eq.get("production")})
