...
class TestVrDeviceFromData(unittest.TestCase):
    ...
    def _from_data(self, name: str, state: object) -> AqualinkDevice:
        return VrDevice.from_data(self.system, _device_data(name, state))
    ...
    def test_binary_sensor_is_on_false(self) -> None:
        dev = VrBinarySensor(self.system, _device_data("running", 0))
        self.assertFalse(dev.is_on)

    # value
    def test_sensor_value_stringified(self) -> None:
        dev = VrSensor(self.system, _device_data("battery", 42))
        self.assertEqual(dev.value, "42")

    def test_error_sensor_value_numeric(self) -> None:
        dev = VrErrorSensor(self.system, _device_data("error_state", 5))
...
def _robot(robot_state: dict[str, Any] | None = None) -> VrRobot:
    system = make_system()
    system._robot_state = robot_state if robot_state is not None else {}
    return VrRobot(system, _device_data("robot", 0))
...
class TestVrRobot(unittest.TestCase):
    def test_from_data_robot_is_robot(self) -> None:
        ...
        system = make_system()
        dev = VrDevice.from_data(system, _device_data("robot", 1))
        self.assertIsInstance(dev, VrRobot)
        self.assertIsInstance(dev, AqualinkVacuum)

    def test_activity_states(self) -> None:
    ...
    def test_activity_error_overrides_state(self) -> None:
        from iaqualink.enums import AqualinkRobotActivity as A
    ...
    def test_fan_speed_reads_prcyc(self) -> None:
        self.assertEqual(
            _robot({"prCyc": 2}).fan_speed, "smart_floor_and_walls"
        )

    def test_fan_speed_none_when_absent(self) -> None:
        self.assertIsNone(_robot({}).fan_speed)

    def test_fan_speed_list(self) -> None:
        self.assertEqual(
...
class TestVrRobotCommands(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_return_delegates(self) -> None:
        ...
        cast(AsyncMock, r.system.return_to_base).assert_awaited_once_with()

    async def test_set_fan_speed_maps_to_cycle(self) -> None:
    ...
    async def test_set_fan_speed_unknown_raises(self) -> None:
    ...
    async def test_set_fan_speed_private_guard_rejects_unknown(self) -> None:
        from iaqualink.exception import AqualinkInvalidParameterException
...
def _sensor(name: str, state: object) -> VrSensor:
    system = make_system()
    # Routes to VrSensor / VrErrorSensor / VrBinarySensor at runtime; cast to
    # the sensor subclass so ty resolves the HA-metadata props the tests assert
    # on (subclasses share device_class/entity_category).
    return cast(
        VrSensor,
        VrDevice.from_data(system, _device_data(name, state)),
    )
...
class TestVrSensorMetadata(unittest.TestCase):
    ...
    def test_time_remaining_sec(self) -> None:
...
class TestVrBinarySensorMetadata(unittest.TestCase):
    def test_running_device_class_and_diagnostic(self) -> None:
        s = _sensor("running", 1)
        self.assertEqual(s.device_class, "running")
        self.assertEqual(s.entity_category, "diagnostic")

    def test_returning_diagnostic(self) -> None:
